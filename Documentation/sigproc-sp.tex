...
\begin{figure}[htbp] 
  \centering
     \includegraphics[width=1\linewidth]{pictures/wiring.jpg}
  \caption{The circuit board during the soldering process}
  \label{fig:soldering}
\end{figure}
...
\item \texttt{1.. ... ...} enables the so called \textit{partz mode} until an other command arrives
\item \texttt{2.. ppp www} enables the \textit{whole weight meassuring mode}, where postion $ p $ will be illuminated until the scale has the weight $w$ grams and then sends \textit{READY} to the Raspberry Pi. \textit{This was used for debugging purposes only.}
...
\item \texttt{11. ttt ttt} waits for $t$ milliseconds that the user touches the scale. I fthe user touched the scale the Arduino sends {TOUCHED} to the Raspberry Pi, else it will send .\textit{NO?INPUT}
...
\begin{minipage}{\linewidth}% to keep image and caption on one page
\makebox[\linewidth]{%        to center the image
   \includegraphics[width=0.5\linewidth, angle =270]{pictures/bar.jpg}}
